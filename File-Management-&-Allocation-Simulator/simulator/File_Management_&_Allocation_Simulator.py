...
class FileManagerApp:
    ...
    def _build_filelist(self, parent):
        ...
        # COLUMNS (mirrors a real OS directory entry):
        #   Bytes  = file size in bytes
        #   Blocks = TOTAL disk blocks the file occupies (index block included)
        #   Length = the file's LENGTH in DATA blocks (the directory "Length"
        #            field from the textbook figures 4.11 / 4.12)
        #   Method = which allocation algorithm stored it
        cols = ("name", "size", "blocks", "length", "method")
        self.tree = ttk.Treeview(card, columns=cols, show="headings",
                                 height=14, selectmode="browse")
        for col, txt, w in (("name", "File", 82), ("size", "Bytes", 46),
                            ("blocks", "Blocks", 46), ("length", "Length", 48),
                            ("method", "Method", 78)):
            self.tree.heading(col, text=txt)
            self.tree.column(col, width=w, anchor="center")
        self.tree.pack(fill="both", expand=True, padx=10, pady=(4, 4))

        # ----- LIVE DIRECTORY ENTRY of the selected file ----------------------
        # This mirrors the "directory" box drawn in figures 4.11-4.13: it shows
        # EXACTLY what fields the OS keeps for the picked file, which differ per
        # allocation method (start+length / start+last / index-block).
        self.dir_entry_lbl = tk.Label(
            card, text="Directory entry: (select a file)",
            bg=C["glass"], fg=C["muted"], justify="left", anchor="w",
            wraplength=340, font=("Consolas", 9))
        self.dir_entry_lbl.pack(fill="x", padx=10, pady=(0, 6))
    ...
    # --------------------------------------------------------------------------
    # --------------------------------------------------------------------------
    # DIRECTORY-ENTRY helpers : return the fields a real OS directory keeps for
    # a file, which are DIFFERENT for each allocation method (see figures
    # 4.11-4.13). Also return the file "Length" in DATA blocks.
    # --------------------------------------------------------------------------
    @staticmethod
    def _data_block_count(f):
        """How many DATA blocks the file uses (index block excluded)."""
        if f["method"] == "Indexed":
            return len(f["index_table"])
        return len(f["blocks"])

    @staticmethod
    def _dir_entry_text(name, f):
        """Textbook directory entry for the chosen allocation method."""
        method = f["method"]
        if method == "Contiguous":
            return (f"Directory entry [Contiguous]\n"
                    f"  File Name       : {name}\n"
                    f"  Start Block No. : {f['start']}\n"
                    f"  Length          : {f['length']} block(s)")
        if method == "Linked":
            blocks = f["blocks"]
            chain = " \u2192 ".join(str(b) for b in blocks) + " \u2192 EOF"
            return (f"Directory entry [Linked]\n"
                    f"  File Name       : {name}\n"
                    f"  Start Block No. : {f['start']}\n"
                    f"  Last Block No.  : {blocks[-1]}\n"
                    f"  Length          : {len(blocks)} block(s)\n"
                    f"  Chain           : {chain}")
        # Indexed
        return (f"Directory entry [Indexed]\n"
                f"  File Name        : {name}\n"
                f"  Index Block No.  : {f['index_block']}\n"
                f"  Index table      : {f['index_table']}\n"
                f"  Length           : {len(f['index_table'])} data block(s)")

    def _show_dir_entry(self):
        """Update the directory-entry label from the current selection."""
        if not hasattr(self, "dir_entry_lbl"):
            return
        sel = self.tree.selection()
        if len(sel) == 1:
            name = str(self.tree.item(sel[0])["values"][0])
            f = self.disk.files.get(name)
            if f:
                self.dir_entry_lbl.config(text=self._dir_entry_text(name, f),
                                          fg=C["text"])
                return
        self.dir_entry_lbl.config(text="Directory entry: (select a file)",
                                  fg=C["muted"])
    ...
    def _build_log(self):
        _, card = self._glass_card(self.root,
                                   ...
                                   side="bottom", fill="x",
                                   padx=12, pady=(0, 10))
        self.log = tk.Text(card, height=8, font=("Consolas", 9),
    ...
    def _update_meter(self):
        ...
        if method == "Indexed":
            per = VirtualDisk.BLOCK_SIZE
            extra = f" + 1 index block = {blocks + 1} total"
            note = f"[ceil({size}/{per}) data blocks, {per} B/block]"
        elif method == "Linked":
            per = VirtualDisk.LINKED_PAYLOAD
            extra = ""
            note = (f"[ceil({size}/{per}), {per} B data/block, "
    ...
    # --------------------------------------------------------------------------
    # 5.0 EXPLAIN : build a short, examiner-friendly reason for HOW MANY blocks
    #     a file occupies, using the exact criteria from the textbook figures.
    #     This is printed to the OS Activity Log on every save so the student
    #     can SEE the arithmetic (like "50 KB / 1 KB = 50 blocks").
    # --------------------------------------------------------------------------
    def _explain_alloc(self, name, size, data_blocks, method, planned):
        BS = VirtualDisk.BLOCK_SIZE
        LP = VirtualDisk.LINKED_PAYLOAD
        PTR = VirtualDisk.POINTER_SIZE
        if method == "Contiguous":
            run = (f"{planned[0]}\u2013{planned[-1]}" if len(planned) > 1
                   else f"{planned[0]}")
            return (
                f"REASON ('{name}' \u2192 Contiguous):\n"
                f"     \u2022 File size            = {size} bytes\n"
                f"     \u2022 Block size           = {BS} bytes (full block holds data)\n"
                f"     \u2022 Blocks needed        = ceil({size} / {BS}) "
                f"= {data_blocks} block(s)\n"
                f"     \u2022 Rule: blocks must be ADJACENT \u2192 one run at "
                f"block(s) {run}\n"
                f"     \u2022 Directory keeps: Start Block = {planned[0]}, "
                f"Length = {data_blocks}")
        if method == "Linked":
            chain = " \u2192 ".join(str(b) for b in planned) + " \u2192 EOF"
            return (
                f"REASON ('{name}' \u2192 Linked):\n"
                f"     \u2022 File size            = {size} bytes\n"
                f"     \u2022 Each block keeps {PTR} bytes for the NEXT-pointer "
                f"\u2192 only {LP} bytes of data per block\n"
                f"     \u2022 Blocks needed        = ceil({size} / {LP}) "
                f"= {data_blocks} block(s)\n"
                f"     \u2022 Rule: blocks may be SCATTERED (no external "
                f"fragmentation) \u2192 chain {chain}\n"
                f"     \u2022 Directory keeps: Start = {planned[0]}, "
                f"Last = {planned[-1]}")
        # Indexed
        total = len(planned)
        return (
            f"REASON ('{name}' \u2192 Indexed):\n"
            f"     \u2022 File size            = {size} bytes\n"
            f"     \u2022 Block size           = {BS} bytes (full block holds data)\n"
            f"     \u2022 Data blocks needed   = ceil({size} / {BS}) "
            f"= {data_blocks} block(s)\n"
            f"     \u2022 + 1 INDEX block to hold the {data_blocks} pointer(s) "
            f"\u2192 {data_blocks} + 1 = {total} block(s) total\n"
            f"     \u2022 Index block {planned[0]} \u2192 data blocks {planned[1:]}\n"
            f"     \u2022 Directory keeps: Index Block Number = {planned[0]}")
    ...
    def on_save(self):
        ...
        # narrate the arithmetic FIRST (why this many blocks), then animate
        self._log(f"Saving '{name}' with {method} allocation ...")
        self._log(self._explain_alloc(name, size, blocks, method, planned))
    ...
    def _animate_allocation(self, name, content, method, planned, i=0):
        ...
        if i < len(planned):
            ...
            self._log(f"DONE: '{name}' saved. Blocks occupied on disk: "
                      f"{planned}")
            self._log(self._dir_entry_text(name, self.disk.files[name]))
            self._refresh_all()
    ...
    def _refresh_table(self):
        ...
        for name, f in self.disk.files.items():
            if query and query not in name.lower():
                continue
            # Length = the directory "Length" field = number of DATA blocks
            length = self._data_block_count(f)
            self.tree.insert("", "end", values=(
                name, f["size_bytes"], len(f["blocks"]), length, f["method"]))
